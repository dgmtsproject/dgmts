  ...
  useEffect(() => {
    const checkProjectAccess = async () => {
      ...
      try {
        ...
        console.error(
          "Unexpected error checking Long Bridge North access:",
          err
        );
    ...
    const checkYellowLineAncAccess = async () => {
      ...
      try {
        ...
        setHasYellowLineAncAccess(
          Boolean(isAdmin || (data && data.length > 0))
        );
    ...
    const fetchProjectNames = async () => {
      try {
        ...
        if (data) {
          const names = {
            longBridge:
              data.find((p) => p.id === 24637)?.name || "Long Bridge North",
            dgmtsTesting:
              data.find((p) => p.id === 20151)?.name || "DGMTS Testing",
            ancDarBc: data.find((p) => p.id === 24429)?.name || "ANC DAR-BC",
            yellowLineAnc:
              data.find((p) => p.id === 25304)?.name || "Yellow Line ANC",
          };
      ...
      <Box>
        ...
        <List>
              ...
              <Collapse in={openGraphs} timeout="auto" unmountOnExit>
                ...
                >
                  ...
                  {(isAdmin || hasDgmtsTestingAccess) && (
                    <>
                      <ListItemButton
                        onClick={handleSecondProjectClick}
                        sx={{ pl: 4 }}
                      >
                        <ListItemIcon
                          sx={{ color: "inherit", minWidth: "36px" }}
                  ...
                  {(isAdmin || hasAncDarBcAccess) && (
                    <>
                      <ListItemButton
                        onClick={handleThirdProjectClick}
                        sx={{ pl: 4 }}
                      >
                        <ListItemIcon
                          sx={{ color: "inherit", minWidth: "36px" }}
                  ...
                  {(isAdmin || hasYellowLineAncAccess) && (
                    <>
                      <ListItemButton
                        onClick={handleFourthProjectClick}
                        sx={{ pl: 4 }}
                      >
                        <ListItemIcon
                          sx={{ color: "inherit", minWidth: "36px" }}
          ...
          {permissions.view_data && (
            <ListItemButton component={Link} to="/data-summary">
              <ListItemIcon sx={{ color: "inherit" }}>
                <SummaryIcon />
              </ListItemIcon>
        ...
        <List>
          <ListItemButton>
            <ListItemIcon sx={{ color: "inherit" }}>
              ...
              >
                {isAdmin ? (
                  <AdminProfileIcon fontSize="small" />
                ) : (
