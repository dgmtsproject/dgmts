...
interface UM15783Data {
  UM15783Readings: UM15783Reading[];
...
const Instantel1Seismograph: React.FC = () => {
  ...
  const [rawData, setRawData] = useState<UM15783Data | null>(null);
  ...
  const processedData = useMemo(() => {
    if (!rawData?.UM15783Readings?.length) {
      return {
        combined: { time: [], x: [], y: [], z: [] },
        x: { time: [], values: [] },
        y: { time: [], values: [] },
    ...
    const filteredReadings = rawData.UM15783Readings.filter(reading => {
    ...
    const getDataValue = (reading: UM15783Reading, axis: 'Longitudinal_PPV' | 'Transverse_PPV' | 'Vertical_PPV' | 'Geophone_PVS') => {
      return reading[axis];
    };
  ...
  const apiPayloadTimeBounds = useMemo(() => {
    const readings = rawData?.UM15783Readings;
  ...
  const dateRangeExcludesAllPoints =
    !!rawData?.UM15783Readings?.length &&
  ...
  /** API responded OK but returned no UM15783 rows for the requested From/To (server-side filter / no files in range). */
  const apiReturnedNoReadingsForRange =
    !error &&
    !loading &&
    rawData !== null &&
    (rawData.UM15783Readings?.length ?? 0) === 0;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        ...
        const baseUrl = 'https://imsite.dullesgeotechnical.com/api/micromate/UM15783/readings';
      ...
      const errorMessage = err instanceof Error
        ? `Failed to fetch UM15783 readings: ${err.message}`
  ...
  const createSinglePlot = (data: { time: Date[]; values: number[] }, axis: string, color: string) => {
    ...
    return (
      <Plot
        ...
        layout={{
          title: { 
            text: `${project?.name || 'Project'} - ${axis} Axis ${dataType === 'ppv' ? 'PPV' : 'Geophone PVS'} Data - ${availableInstruments.length > 0 && availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location ? availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location : 'Location: None'}`, 
  ...
  const createSoundDbPlot = (soundDb: { time: Date[]; l10: number[]; l90: number[]; lMax: number[] }) => {
    ...
    return (
      <Plot
        ...
        layout={{
          title: { 
            text: `${project?.name || 'Project'} - Sound (db) Data - ${availableInstruments.length > 0 && availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location ? availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location : 'Location: None'}`, 
            font: { size: 20, weight: 700, color: '#003087' },
            x: 0.5,
            xanchor: 'center'
          },
  ...
  const createCombinedPlot = (combined: { time: Date[]; x: number[]; y: number[]; z: number[] }) => {
    ...
    return (
      <Plot
        ...
        layout={{
          title: { 
            text: `${project?.name || 'Project'} - Combined PPV Data - ${availableInstruments.length > 0 && availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location ? availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location : 'Location: None'}`, 
            font: { size: 20, weight: 700, color: '#003087' },
            x: 0.5,
            xanchor: 'center'
          },
      ...
      <MainContentWrapper>
        ...
        <Box p={3}>
          <Typography variant="h4" align="center" sx={{ mb: 3, mt: 2 }}>
            {project ? `${project.name} - Data Graphs (Instantel 1)` : 'Data Graphs (Instantel 1)'}
          </Typography>
          ...
          <LocalizationProvider dateAdapter={AdapterDateFns}>
            <Stack direction="row" spacing={2} sx={{ mb: 4 }} alignItems="center" flexWrap="wrap">
              ...
              {rawData && rawData.UM15783Readings.length > 0 && (
                <Button 
                  variant="outlined" 
                  onClick={() => {
          ...
          {apiReturnedNoReadingsForRange && (
            <Alert severity="warning" sx={{ mb: 3 }}>
              ...
              service did not find any Instantel 1 (UM15783) readings in that window—try <strong>older</strong> dates,
                      ...
                      <Tooltip title="Open in Popup">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            const chartLayout = {
                              title: { 
                                text: `${project?.name || 'Project'} - Geophone PVS Data - ${availableInstruments.length > 0 && availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location ? availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location : 'Location: None'}`,
                                font: { size: 20, weight: 'bold', color: '#003087' },
                                x: 0.5,
                                xanchor: 'center'
                              },
                        ...
                        <Tooltip title="Open in Popup">
                          <Button
                            ...
                            onClick={() => {
                              ...
                              const chartLayout = {
                                title: { 
                                  text: `${project?.name || 'Project'} - X (Longitudinal) Axis PPV Data - ${availableInstruments.length > 0 && availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location ? availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location : 'Location: None'}`,
                                  font: { size: 20, weight: 'bold', color: '#003087' },
                                  x: 0.5,
                                  xanchor: 'center'
                                },
                        ...
                        <Tooltip title="Open in Popup">
                          <Button
                            ...
                            onClick={() => {
                              ...
                              const chartLayout = {
                                title: { 
                                  text: `${project?.name || 'Project'} - Y (Transverse) Axis PPV Data - ${availableInstruments.length > 0 && availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location ? availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location : 'Location: None'}`,
                                  font: { size: 20, weight: 'bold', color: '#003087' },
                                  x: 0.5,
                                  xanchor: 'center'
                                },
                        ...
                        <Tooltip title="Open in Popup">
                          <Button
                            ...
                            onClick={() => {
                              ...
                              const chartLayout = {
                                title: { 
                                  text: `${project?.name || 'Project'} - Z (Vertical) Axis PPV Data - ${availableInstruments.length > 0 && availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location ? availableInstruments.find(inst => inst.instrument_id === 'Instantel 1')?.instrument_location : 'Location: None'}`,
                                  font: { size: 20, weight: 'bold', color: '#003087' },
                                  x: 0.5,
                                  xanchor: 'center'
                                },
          ...
          {!loading && (!rawData || rawData.UM15783Readings.length === 0) && !error && (
          ...
          {instrumentSettings && (
            <Box mt={2} p={2} bgcolor="grey.100" borderRadius={1}>
              <Typography variant="h6" gutterBottom>
                {project ? `${project.name} - Reference Levels (Instantel 1)` : 'Reference Levels (Instantel 1)'}
              </Typography>
