...
const MICROMATE_DEVICE = 'UM15783';
...
const Instantel1Seismograph: React.FC = () => {
  ...
  const [activeInstrumentId, setActiveInstrumentId] = useState<string>(
    location.state?.instrumentId || 'Instantel 1'
  );
  ...
  const fetchProjectInfo = async () => {
    try {
      const { data: instrumentRows, error: instrumentError } = await supabase
        ...
        .or(`instrument_name.eq.${MICROMATE_DEVICE},instrument_id.eq.Instantel 1,sno.eq.${MICROMATE_DEVICE}`);
      ...
      if (!instrumentData?.project_id) {
        console.error('No UM15783 instrument found in database');
        return;
      }
