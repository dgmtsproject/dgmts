  ...
  const createSinglePlot = (data: { time: Date[]; values: number[] }, axis: string, color: string) => {
    ...
    return (
      <Plot
        ...
        layout={{
          title: { 
            text: `${project?.name || 'Project'} - ${axis} Axis Vibration Data - ${availableInstruments.length > 0 && availableInstruments.find(inst => inst.instrument_id === 'ROCKSMG-2')?.instrument_location ? availableInstruments.find(inst => inst.instrument_id === 'ROCKSMG-2')?.instrument_location : 'Location: None'}`, 
          ...
          xaxis: {
            title: { 
              text: `Time<br><span style="font-size:12px;color:#666;">ROCKSMG-2</span>`, 
